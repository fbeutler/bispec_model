...
def Om(z=para.redshift):
	''' Return \Omega_m(z) '''
	return para.cosmo.Omega_m(z)
...
def A(z=para.redshift):
	...
	- chi has unit (Mpc/h)
	'''
	return bevo(z) + 1.5*Om(z) - 3. + (2. - 5.*LumS(z))*(1. - 1./(chi(z)*aHz_in_invdist(z)))
...
def F2(k1, k2, k3):
	...
	mu = k1dk2/(k1*k2)
	return 5./7. + 0.5*k1dk2*(1./(k1**2) + 1./(k2**2)) + 2.*mu**2/7.
...
def G2(k1, k2, k3):
	...
	k1dk2 = (k3**2 - k1**2 - k2**2)/2.
	mu = k1dk2/(k1*k2)
	return 3./7. + 0.5*k1dk2*(1./k1**2 + 1./k2**2) + 4.*mu**2/7.
...
def KD2(z, k1, k2, k3, mu1, mu2, mu3): 
	...
	line1 = -1.5*(mu1*k1/k2**2 + mu2*k2/k1**2)*Om(z)*B10(z) + 2.*mu12*(mu1/k2 + mu2/k1)*fg(z)**2 + (mu1/k1 + mu2/k2)*C(z)*fg(z)
	line2 = -1.5*(mu1**3*k1/k2**2 + mu2**3*k2/k1**2)*Om(z)*fg(z) + mu1*mu2*(mu1/k2 + mu2/k1)*(1.5*Om(z) - EA(z)*fg(z))*fg(z)
...
def BNgggintegrand(Omega, Phi, z, k1, k2, k3, P1, P2, P3):
	...
	return np.real(BgN*sp.sph_harm(0, 0, Omega, Phi))/math.sqrt(4.*np.pi)
...
def BDgggintegrand(Omega, Phi, z, k1, k2, k3, P1, P2, P3, L, M):
	...
	return np.real(BgD*sp.sph_harm(M, L, Phi, Omega))/math.sqrt(12.*np.pi)

def BNggg(z, k1, k2, k3, P1, P2, P3):
	''' 
	...
	result = integrate.dblquad(BNgggintegrand,       # func
							   0.002, 	             # \phi lower limit
							   2.*np.pi-ep, 	     # \phi upper limit
							   lambda Phi: 0.002,    # \Omega lower limit
							   lambda Phi: np.pi-ep, # \Omega upper limit
							   args=(z, k1, k2, k3, P1, P2, P3), epsabs=1.49e-8, epsrel=1.49e-8)
...
def BDggg(z, k1, k2, k3, P1, P2, P3, L, M):
	...
	'''
	time0 = time.time()
	result = integrate.nquad(BDgggintegrand, [[0.002, 2.*np.pi-ep], [0.002, np.pi-ep]], args=(z, k1, k2, k3, P1, P2, P3, L, M), full_output=True)
...
def Bg(z, k1, k2, Theta_12):
	...
	for i in range(len(Theta_12)): 
		...
		BgN[i] = BNggg(z, k1, k2, k3[i], pk_k1, pk_k2, para.Pk_class(k3[i]))[0]
		BgDm0[i] = BDggg(z, k1, k2, k3[i], pk_k1, pk_k2, para.Pk_class(k3[i]), 1, 0)[0]
		BgDm1[i] = BDggg(z, k1, k2, k3[i], pk_k1, pk_k2, para.Pk_class(k3[i]), 1, 1)[0]
	print("Finished Bmonopole_sq calculation after %0.8fsec" % (time.time()-time0))	                                                                              
...
def plot_bi(theta_12, Bmono_N, BDm0, BDm1):
	...
	plt.plot(theta_12, Bmono_N/para.cosmo.h**6, label=r'$l = 0$, Newtonian')
	...
	plt.plot(theta_12, abs(BDm0 + BDm1)/para.cosmo.h**6, label=r'$l = 1$, $m = \pm1$')
	plt.yscale('log')
	plt.legend(loc='best')
